...
export const viteReactPrompt: FrameworkPrompt = {
  ...
  envConventions: [
    ...
    "   - At BUILD time: create the schema with the `data.migrate` tool (one call per table, e.g. migration_id `0001_init_tasks`). Use `CREATE TABLE IF NOT EXISTS` so re-running is safe. Include the `created_by` column + RLS policy from the template. Then inspect with `data.schema` before writing app code.",
