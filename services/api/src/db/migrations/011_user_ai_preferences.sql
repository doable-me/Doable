...
ALTER TABLE workspace_ai_settings
    ADD COLUMN enforce_ai boolean NOT NULL DEFAULT false,
    ADD COLUMN enforced_copilot_account_id uuid REFERENCES github_copilot_accounts(id) ON DELETE SET NULL,
    ADD COLUMN enforced_provider_id uuid REFERENCES ai_providers(id) ON DELETE SET NULL,
    ADD COLUMN enforced_model text;
...
CREATE INDEX idx_uap_user ON user_ai_preferences(user_id);
