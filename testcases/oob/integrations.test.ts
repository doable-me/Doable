...
export async function runIntegrationsTests(ownerToken: string, workspaceId: string | null): Promise<void> {
  ...
  // TC-IN02: GET /api/integrations returns array (connection list for user)
  try {
    const res = await apiFetch("/api/integrations", { token: ownerToken });
    const body = await res.json() as Record<string, unknown>;
    saveEvidence("TC-IN02", body, Object.fromEntries(res.headers.entries()));
    assert(res.status === 200, `Expected 200, got ${res.status}`);
    const list = (body.data ?? body.connections ?? body) as unknown[];
    assert(Array.isArray(list), `Expected array from /integrations, got ${typeof list}`);
    pass("TC-IN02", "GET /api/integrations returns 200 with array");
  } catch (e) {
    fail("TC-IN02", "GET /api/integrations returns 200 with array", (e as Error).message);
  }

  // TC-IN03: GET /api/integrations without token returns 401
  try {
    ...
    pass("TC-IN03", "GET /api/integrations without token returns 401/403");
  } catch (e) {
    fail("TC-IN03", "GET /api/integrations without token returns 401/403", (e as Error).message);
  }

  // TC-IN04: GET /api/integrations/connections returns connection list
  try {
    const res = await apiFetch("/api/integrations/connections", { token: ownerToken });
    const body = await res.json() as Record<string, unknown>;
  ...
  // TC-IN06: DELETE /api/integrations/connections/:id with unknown id returns 404
  try {
    const res = await apiFetch("/api/integrations/connections/nonexistent-id-000", {
      method: "DELETE",
      token: ownerToken,
    });
    const text = await res.text();
