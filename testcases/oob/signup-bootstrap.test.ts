...
export async function runSignupBootstrapTests(): Promise<void> {
  ...
  try {
    ...
    if (res.status === 409) {
      ...
      const loginTokens = (loginBody.tokens as Record<string, unknown>) ?? loginBody;
      ownerToken = (loginTokens.accessToken ?? loginTokens.token) as string ?? null;
    ...
    const ownerTokens = (body.tokens as Record<string, unknown>) ?? body;
    ownerToken  = (ownerTokens.accessToken ?? ownerTokens.token) as string ?? null;
  ...
  try {
    ...
    const secondTokens = (body.tokens as Record<string, unknown>) ?? body;
    secondToken  = (secondTokens.accessToken ?? secondTokens.token) as string ?? null;
