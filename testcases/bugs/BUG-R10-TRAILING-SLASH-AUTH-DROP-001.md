...
- **Status**: FIXED in R12 PR #35 + R12 verified (2026-05-15) — commit `166c94af` on `fix/r12-trailing-slash-no-redirect`; top-level Hono app + every sub-router switched to `{ strict: false }`, 308-redirect middleware deleted. All 6 trailing-slash paths (/templates/, /workspaces/, /projects/) confirmed returning 200 with Authorization header preserved.
