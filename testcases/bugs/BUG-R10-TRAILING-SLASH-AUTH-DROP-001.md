...
- **Status**: FIXED (2026-05-15, Ralph R12, branch `fix/r12-trailing-slash-no-redirect`) — top-level Hono app + every sub-router switched to `{ strict: false }`, the 308-redirect middleware deleted. `/foo` and `/foo/` now route to the same handler with no redirect, so no client can drop Authorization on the followed request.
