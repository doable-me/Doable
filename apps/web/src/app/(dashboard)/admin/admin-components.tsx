    ...
    <div className="rounded-lg border border-zinc-800 bg-zinc-900/50">
      <div className="flex items-center gap-3 px-4 py-3">
        <button onClick={() => onToggle(feature.feature_key, !feature.enabled)} className="shrink-0">
          {feature.enabled ? <ToggleRight className="h-6 w-6 text-brand-500" /> : <ToggleLeft className="h-6 w-6 text-zinc-600" />}
        </button>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className={`text-sm font-medium ${feature.enabled ? "text-white" : "text-zinc-500"}`}>{feature.label}</span>
            <code className="text-[10px] text-zinc-600 bg-zinc-800 px-1.5 py-0.5 rounded">{feature.feature_key}</code>
          </div>
          {feature.description && <p className="text-xs text-zinc-500 mt-0.5">{feature.description}</p>}
        </div>
        ...
        <button onClick={() => setExpanded(!expanded)} className="p-1 rounded hover:bg-zinc-800 text-zinc-500 hover:text-zinc-300 transition-colors">
      ...
      {expanded && (
        <div className="border-t border-zinc-800 px-4 py-3 flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <select value={feature.min_plan ?? ""} onChange={(e) => onUpdate(feature.feature_key, { min_plan: e.target.value || null })} className="rounded-md bg-zinc-800 border border-zinc-700 text-xs text-zinc-300 px-2 py-1 outline-none focus:border-brand-500">
              {PLAN_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
            </select>
          </div>
          <div className="flex items-center gap-2">
            <label className="text-xs text-zinc-400">Min Role:</label>
            <select value={feature.min_role ?? ""} onChange={(e) => onUpdate(feature.feature_key, { min_role: e.target.value || null })} className="rounded-md bg-zinc-800 border border-zinc-700 text-xs text-zinc-300 px-2 py-1 outline-none focus:border-brand-500">
  ...
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-900/50">
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <p className="text-sm font-medium text-white truncate">{u.display_name ?? u.email.split("@")[0]}</p>
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button onClick={startEditCredits} className="rounded p-1.5 text-zinc-500 hover:text-emerald-400 hover:bg-zinc-800 transition-colors" title="Manage credits"><Zap className="h-4 w-4" /></button>
          <button onClick={startEdit} className="rounded p-1.5 text-zinc-500 hover:text-brand-400 hover:bg-zinc-800 transition-colors" title="Configure AI for this user"><Bot className="h-4 w-4" /></button>
          {hasAllocation && (
            <button onClick={() => onReset(u.user_id)} className="rounded p-1.5 text-zinc-500 hover:text-amber-400 hover:bg-zinc-800 transition-colors" title="Reset AI allocation"><RotateCcw className="h-3.5 w-3.5" /></button>
          )}
        </div>
        <select value={u.platform_role ?? "member"} onChange={(e) => onChangeRole(u.user_id, e.target.value)} disabled={isSelf} className={`shrink-0 rounded-md bg-zinc-800 border border-zinc-700 text-xs font-medium px-2 py-1.5 outline-none focus:border-brand-500 disabled:opacity-40 disabled:cursor-not-allowed ${ROLE_COLORS[u.platform_role ?? "member"] ?? "text-zinc-300"}`}>
          {WORKSPACE_ROLES.map((r) => <option key={r} value={r}>{ROLE_LABELS[r]}</option>)}
        </select>
        <select value={u.workspace_plan ?? "free"} onChange={(e) => onChangePlan(u.user_id, e.target.value)} className={`shrink-0 rounded-md bg-zinc-800 border border-zinc-700 text-xs font-medium px-2 py-1.5 outline-none focus:border-brand-500 ${PLAN_COLORS[u.workspace_plan ?? "free"] ?? "text-zinc-300"}`}>
      ...
      {editing && (
        <div className="border-t border-zinc-800 px-4 py-4">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-medium text-zinc-300">Configure AI for {u.display_name ?? u.email}</p>
            <button onClick={() => setEditing(false)} className="text-zinc-500 hover:text-zinc-300"><X className="h-3.5 w-3.5" /></button>
          </div>
          <div className="mb-3">
            <div className="flex rounded-lg border border-zinc-700 overflow-hidden w-fit">
              <button onClick={() => setSource("copilot")} className={`px-3 py-1.5 text-xs font-medium transition-colors ${source === "copilot" ? "bg-brand-600 text-white" : "bg-zinc-800 text-zinc-400 hover:text-zinc-200"}`}>GitHub Copilot</button>
              <button onClick={() => setSource("custom")} className={`px-3 py-1.5 text-xs font-medium transition-colors ${source === "custom" ? "bg-brand-600 text-white" : "bg-zinc-800 text-zinc-400 hover:text-zinc-200"}`}>Custom Provider</button>
            </div>
            <p className="text-[10px] text-zinc-500 mt-1">Both configurations are saved. The selected tab is what this user will use.</p>
          </div>
            ...
            {source === "copilot" ? (
              <>
                <div>
                  ...
                  <select value={copilotAccountId} onChange={(e) => setCopilotAccountId(e.target.value)} className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-1.5 text-xs text-zinc-200 outline-none focus:border-brand-500">
                ...
                <div>
                  ...
                  <input type="text" value={copilotModel} onChange={(e) => setCopilotModel(e.target.value)} placeholder="e.g. claude-sonnet-4 (blank = auto)" className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-1.5 text-xs text-zinc-200 placeholder:text-zinc-500 outline-none focus:border-brand-500" />
                </div>
              </>
            ) : (
              <>
                <div>
                  ...
                  <select value={providerId} onChange={(e) => setProviderId(e.target.value)} className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-1.5 text-xs text-zinc-200 outline-none focus:border-brand-500">
                ...
                <div>
                  <label className="block text-[10px] font-medium text-zinc-400 mb-1 uppercase tracking-wider">Model</label>
                  <input type="text" value={providerModel} onChange={(e) => setProviderModel(e.target.value)} placeholder="e.g. gpt-4o" className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-1.5 text-xs text-zinc-200 placeholder:text-zinc-500 outline-none focus:border-brand-500" />
                </div>
              </>
            )}
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setEditing(false)} className="rounded-lg px-3 py-1.5 text-xs font-medium text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 transition-colors">Cancel</button>
          </div>
        </div>
      )}
      ...
      {editingCredits && (
        <div className="border-t border-zinc-800 px-4 py-4">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-medium text-zinc-300"><Zap className="inline h-3.5 w-3.5 text-emerald-400 mr-1" />Credits for {u.display_name ?? u.email}</p>
            <button onClick={() => setEditingCredits(false)} className="text-zinc-500 hover:text-zinc-300"><X className="h-3.5 w-3.5" /></button>
          </div>
          {creditLoading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-3 mb-3">
                <div>
                  ...
                  <input type="number" min={0} value={creditDaily} onChange={(e) => setCreditDaily(Number(e.target.value))} className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-1.5 text-xs text-zinc-200 outline-none focus:border-brand-500" />
                </div>
                <div>
                  <label className="block text-[10px] font-medium text-zinc-400 mb-1 uppercase tracking-wider">Monthly Credits</label>
                  <input type="number" min={0} value={creditMonthly} onChange={(e) => setCreditMonthly(Number(e.target.value))} className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-1.5 text-xs text-zinc-200 outline-none focus:border-brand-500" />
                </div>
                <div>
                  <label className="block text-[10px] font-medium text-zinc-400 mb-1 uppercase tracking-wider">Rollover Credits</label>
                  <input type="number" min={0} value={creditRollover} onChange={(e) => setCreditRollover(Number(e.target.value))} className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-1.5 text-xs text-zinc-200 outline-none focus:border-brand-500" />
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setEditingCredits(false)} className="rounded-lg px-3 py-1.5 text-xs font-medium text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 transition-colors">Cancel</button>
              </div>
            </>
          )}
        </div>
