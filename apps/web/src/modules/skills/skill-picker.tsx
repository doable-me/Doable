...
export function SkillPickerButton({ manifest, onSelect, disabled }: SkillPickerButtonProps) {
  ...
  return (
    <div ref={containerRef} className="relative shrink-0">
      ...
      {/* Popover */}
      {open && (
        <div className="absolute bottom-full left-0 mb-2 w-72 rounded-lg border bg-popover shadow-xl z-50 overflow-hidden">
          {/* Header */}
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((skill) => (
                ...
                >
                  ...
                  {!skill.auto_invoke && (
                    <span className="text-[10px] text-muted-foreground/60 mt-0.5 shrink-0">manual</span>
                  )}
                </button>
              ))
            )}
