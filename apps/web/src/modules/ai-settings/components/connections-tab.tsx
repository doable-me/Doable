            ...
            <div className="flex gap-1.5">
              {(["openai", "anthropic", "azure"] as const).map((t) => (
                <button key={t} onClick={() => handleProviderTypeChange(t)}
                  className={`rounded-lg px-2.5 py-1 text-xs font-medium transition-colors ${providerType === t ? "bg-brand-600 text-white" : "bg-zinc-800 text-zinc-400 hover:text-zinc-200"}`}>
