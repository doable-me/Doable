#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════════╗
# ║  Doable — generic env, secure-by-default Ubuntu setup (v3)           ║
# ║                                                                      ║
# ║  Env-driven: pass DOABLE_ENV_NAME, DOABLE_DOMAIN, etc. via env vars. ║
# ║  Hosts:    <env>.<domain> / <env>-api.<domain> / <env>-ws.<domain>  ║
...
#
# Usage:
#   sudo DOABLE_ENV_NAME=myorg bash /tmp/setup-server-v3.sh
#
...
ENV_NAME="${DOABLE_ENV_NAME:?Must set DOABLE_ENV_NAME (e.g. myorg, qa, prod)}"
DOABLE_DOMAIN="${DOABLE_DOMAIN:-doable.me}"
APP_DIR="${DOABLE_APP_DIR:-/opt/doable}"
APP_USER="${DOABLE_APP_USER:-doable}"
APP_GROUP="${DOABLE_APP_GROUP:-doable}"
APP_LOG_DIR="${DOABLE_APP_LOG_DIR:-/var/log/doable}"
ADMIN_USER="${DOABLE_ADMIN_USER:-douser}"
PUBLISH_PREFIX="${DOABLE_PUBLISH_PREFIX:-${ENV_NAME}-}"
NODE_MAJOR="${DOABLE_NODE_MAJOR:-22}"
PNPM_VERSION="${DOABLE_PNPM_VERSION:-9.15.4}"
WITH_TMUX="${DOABLE_WITH_TMUX:-0}"
NON_INTERACTIVE="${DOABLE_NON_INTERACTIVE:-0}"
SKIP_ADMIN_USER="${DOABLE_SKIP_ADMIN_USER:-0}"
...
# Default shell: nologin (secure). If WITH_TMUX=1, switch to /bin/bash so
# the operator can `sudo -u doable tmux attach`.
...
if ! id -u "${APP_USER}" >/dev/null 2>&1; then
  info "Creating system user ${APP_USER} (shell=${DEFAULT_SHELL})..."
  useradd --system --shell "${DEFAULT_SHELL}" \
    --home-dir "${APP_DIR}" --create-home \
    --user-group "${APP_USER}"
  ok "Created ${APP_USER} (uid=$(id -u "${APP_USER}"))"
else
  # Adjust shell on rerun in case WITH_TMUX flipped.
  current_shell="$(getent passwd "${APP_USER}" | cut -d: -f7)"
  if [ "${current_shell}" != "${DEFAULT_SHELL}" ]; then
    info "Updating ${APP_USER} shell ${current_shell} -> ${DEFAULT_SHELL}"
    usermod -s "${DEFAULT_SHELL}" "${APP_USER}"
  fi
  ok "User ${APP_USER} already exists (uid=$(id -u "${APP_USER}"), shell=${DEFAULT_SHELL})"
fi
...
if [ "$SKIP_ADMIN_USER" != "1" ] && [ -n "$ADMIN_USER" ]; then
  ...
  warn "SKIP_ADMIN_USER=1 (or empty ADMIN_USER) — not provisioning a sudo admin user."
  warn "  If you skip this, you MUST ensure another non-root sudo account exists with a working SSH key"
  warn "  BEFORE Phase 12 hardening disables root SSH login. Otherwise you risk being locked out."
  warn "  To remediate post-run: useradd -m -s /bin/bash <name>; usermod -aG sudo <name>;"
  warn "  echo '<name> ALL=(ALL) NOPASSWD:ALL' >/etc/sudoers.d/90-<name>; install authorized_keys."
fi
...
phase "Phase 7/15  Caddy *.${DOABLE_DOMAIN} wildcard server (loopback)"

# Sites root for published projects (static + framework dist output).
install -d -o "${APP_USER}" -g "${APP_GROUP}" -m 0755 "${APP_DIR}/sites"

# Caddyfile binds 127.0.0.1:8080; cloudflared sends *.<domain> here.
...
# Non-interactive cert install path: if DOABLE_CF_TUNNEL_UUID is provided
# AND a cert source is available, write /root/.cloudflared/cert.pem and
# skip the interactive `cloudflared tunnel login` step.
CF_AUTOWIRED=0
if [ -n "${DOABLE_CF_TUNNEL_UUID:-}" ]; then
  install -d -o root -g root -m 0700 /root/.cloudflared
  install -d -o root -g root -m 0755 /etc/cloudflared
  # cert.pem authorises operations against the CF account.
  if [ -n "${DOABLE_CF_CERT_PEM:-}" ]; then
    umask 077
    printf '%s' "${DOABLE_CF_CERT_PEM}" > /root/.cloudflared/cert.pem
    chmod 0600 /root/.cloudflared/cert.pem; chown root:root /root/.cloudflared/cert.pem
    CF_AUTOWIRED=1
    ok "Installed cloudflared cert from DOABLE_CF_CERT_PEM env"
  elif [ -n "${DOABLE_CF_CERT_PEM_PATH:-}" ] && [ -f "${DOABLE_CF_CERT_PEM_PATH}" ]; then
    install -o root -g root -m 0600 "${DOABLE_CF_CERT_PEM_PATH}" /root/.cloudflared/cert.pem
    CF_AUTOWIRED=1
    ok "Installed cloudflared cert from ${DOABLE_CF_CERT_PEM_PATH}"
  elif [ -n "${DOABLE_CF_CERT_PEM_B64:-}" ]; then
    umask 077
    echo "${DOABLE_CF_CERT_PEM_B64}" | base64 -d > /root/.cloudflared/cert.pem
    chmod 0600 /root/.cloudflared/cert.pem; chown root:root /root/.cloudflared/cert.pem
    CF_AUTOWIRED=1
    ok "Installed cloudflared cert from DOABLE_CF_CERT_PEM_B64 env"
  else
    warn "DOABLE_CF_TUNNEL_UUID set but no cert source provided (DOABLE_CF_CERT_PEM / _PATH / _B64) — cert NOT installed."
  fi

  # Per-tunnel credentials JSON — required for cloudflared to actually run.
  if [ -n "${DOABLE_CF_CREDENTIALS_JSON:-}" ]; then
    umask 077
    printf '%s' "${DOABLE_CF_CREDENTIALS_JSON}" > "/etc/cloudflared/${DOABLE_CF_TUNNEL_UUID}.json"
    chmod 0600 "/etc/cloudflared/${DOABLE_CF_TUNNEL_UUID}.json"
    chown root:root "/etc/cloudflared/${DOABLE_CF_TUNNEL_UUID}.json"
    ok "Installed /etc/cloudflared/${DOABLE_CF_TUNNEL_UUID}.json"
  elif [ -n "${DOABLE_CF_CREDENTIALS_JSON_PATH:-}" ] && [ -f "${DOABLE_CF_CREDENTIALS_JSON_PATH}" ]; then
    install -o root -g root -m 0600 "${DOABLE_CF_CREDENTIALS_JSON_PATH}" "/etc/cloudflared/${DOABLE_CF_TUNNEL_UUID}.json"
    ok "Installed credentials JSON from ${DOABLE_CF_CREDENTIALS_JSON_PATH}"
  else
    warn "DOABLE_CF_TUNNEL_UUID set but DOABLE_CF_CREDENTIALS_JSON not provided — service won't start until operator drops <uuid>.json into /etc/cloudflared/"
    CF_AUTOWIRED=0
  fi
elif [ "${NON_INTERACTIVE}" = "1" ]; then
  err "DOABLE_NON_INTERACTIVE=1 but DOABLE_CF_TUNNEL_UUID is empty. Set DOABLE_CF_TUNNEL_UUID, DOABLE_CF_CERT_PEM (or _PATH/_B64), DOABLE_CF_CREDENTIALS_JSON, OR rerun interactively."
fi

if [ ! -f /etc/cloudflared/config.yml ]; then
  if [ "${CF_AUTOWIRED}" = "1" ]; then
    cat > /etc/cloudflared/config.yml <<CFCONF
# Doable cloudflared ingress for the ${ENV_NAME} environment.
# Auto-wired via DOABLE_CF_TUNNEL_UUID env.
tunnel: ${DOABLE_CF_TUNNEL_UUID}
credentials-file: /etc/cloudflared/${DOABLE_CF_TUNNEL_UUID}.json
...
CFCONF
    ok "Wrote /etc/cloudflared/config.yml with tunnel UUID ${DOABLE_CF_TUNNEL_UUID}"
  else
    cat > /etc/cloudflared/config.yml <<CFCONF
# Doable cloudflared ingress for the ${ENV_NAME} environment.
...
# If autowired, validate ingress + install service + start it. Otherwise
# leave it disabled — operator will wire it manually after `cloudflared tunnel login`.
if [ "${CF_AUTOWIRED}" = "1" ]; then
  if cloudflared --config /etc/cloudflared/config.yml tunnel ingress validate >/dev/null 2>&1; then
    ...
    systemctl enable --now cloudflared || warn "cloudflared service install/enable returned non-zero — inspect 'systemctl status cloudflared'"
    ok "cloudflared service enabled (autowired tunnel ${DOABLE_CF_TUNNEL_UUID})"
  else
    warn "cloudflared ingress validation failed — service NOT enabled. Inspect /etc/cloudflared/config.yml"
  fi
else
  ok "cloudflared installed; service NOT enabled (operator step — see manual checklist)."
fi
...
CLOUDFLARED_TUNNEL_ID=${DOABLE_CF_TUNNEL_UUID:-PENDING_FROM_OPERATOR}
...
# AI provider API calls — through Squid. That broke BYOK providers since
# their hostnames weren't in the allowlist. Fix: only BUILD_HTTP_PROXY,
# never HTTP_PROXY/HTTPS_PROXY at the parent process level. Each child
# spawn (vite-jail, runNpmInstall) injects HTTP_PROXY itself when needed.
...
# ─── Phase 9.5: Pre-migration housekeeping ───────────────────────────────
phase "Phase 9.5/15  Pre-migration housekeeping (rename stale migration filenames)"

# Some pre-existing repo snapshots ship with packages/db/migrations/055_api_key_tool_scoping.sql
# but the canonical numbering puts it at 067. Rename it in place if found
# BEFORE migrations run, so the apply order is correct. Idempotent: no-op
# if the target already exists or the source is already renamed.
STALE_MIGRATION="${APP_DIR}/packages/db/migrations/055_api_key_tool_scoping.sql"
NEW_MIGRATION="${APP_DIR}/packages/db/migrations/067_api_key_tool_scoping.sql"
if [ -f "${STALE_MIGRATION}" ] && [ ! -f "${NEW_MIGRATION}" ]; then
  info "Renaming stale migration 055 -> 067 api_key_tool_scoping.sql"
  mv "${STALE_MIGRATION}" "${NEW_MIGRATION}"
  chown "${APP_USER}:${APP_GROUP}" "${NEW_MIGRATION}" || true
  ok "Renamed ${STALE_MIGRATION} -> ${NEW_MIGRATION}"
elif [ -f "${NEW_MIGRATION}" ]; then
  ok "Migration already at canonical name 067_api_key_tool_scoping.sql"
else
  ok "No stale migration found (repo not yet on disk, or already canonical)."
fi
...
# fresh env host but the kernel doesn't require named users for setpriv,
...
# Common hardening block (PrivateTmp REMOVED per v3 spec).
# These three units share most settings; the differences are
# WorkingDirectory + ExecStart + (web-only) ExecStartPre/Environment.

cat > /etc/systemd/system/doable-api.service <<SVCEOF
[Unit]
Description=Doable API service
After=network-online.target postgresql.service
Wants=network-online.target postgresql.service
PartOf=doable.target
...
[Install]
WantedBy=doable.target
SVCEOF

cat > /etc/systemd/system/doable-web.service <<SVCEOF
[Unit]
Description=Doable Web (Next.js)
After=network-online.target postgresql.service
Wants=network-online.target postgresql.service
PartOf=doable.target
...
ExecStartPre=/usr/bin/pnpm --filter web build
ExecStart=/usr/bin/node .next/standalone/apps/web/server.js
StandardOutput=journal
StandardError=journal
SyslogIdentifier=doable-web
Restart=on-failure
RestartSec=10
...
cat > /etc/systemd/system/doable-ws.service <<SVCEOF
[Unit]
Description=Doable WebSocket service
After=network-online.target postgresql.service
Wants=network-online.target postgresql.service
PartOf=doable.target
...
User=${APP_USER}
Group=${APP_GROUP}
WorkingDirectory=${APP_DIR}/services/ws
EnvironmentFile=${APP_DIR}/.env
Environment=PATH=/usr/local/bin:/usr/bin:/bin
ExecStart=/usr/bin/pnpm dev:ws
StandardOutput=journal
StandardError=journal
SyslogIdentifier=doable-ws
Restart=on-failure
RestartSec=10
...
cat > /etc/systemd/system/doable.target <<'TGTEOF'
[Unit]
Description=Doable platform (api + web + ws)
Wants=doable-api.service doable-web.service doable-ws.service
After=doable-api.service doable-web.service doable-ws.service

[Install]
WantedBy=multi-user.target
TGTEOF

# Optional: legacy tmux unit (gated by WITH_TMUX=1), for operators who want
# to attach into a long-lived shell session showing all 3 services. Note:
# this is a separate path; the per-service units above are still installed.
if [ "$WITH_TMUX" = "1" ]; then
  info "WITH_TMUX=1 — also installing /etc/systemd/system/doable-tmux.service"
  cat > /etc/systemd/system/doable-tmux.service <<SVCEOF
[Unit]
Description=Doable platform (api + web + ws via tmux) — optional convenience unit
After=network-online.target postgresql.service
Wants=network-online.target postgresql.service
...
WantedBy=multi-user.target
SVCEOF
  ok "Installed doable-tmux.service (operator may enable it manually if desired)."
fi

systemctl daemon-reload
systemctl enable doable-api.service doable-web.service doable-ws.service doable.target
ok "doable-api/web/ws + doable.target installed; NOT started (operator scp's repo first)."
...
╔══════════════════════════════════════════════════════════════════════════╗
║  setup-server-v3.sh: BASE SYSTEM READY (env=${ENV_NAME}).                ║
║  Manual steps remaining below.                                           ║
╚══════════════════════════════════════════════════════════════════════════╝

1. SSH lockdown (do this FIRST — script staged, didn't restart sshd):
     # From your laptop, in a NEW terminal:
     ssh ${ADMIN_USER}@<server-ip>          # must succeed with key auth
     ssh ubuntu@<server-ip>                 # confirm fallback works
     # Then on the server:
...
2. scp the Doable repo into ${APP_DIR} (don't use 'gh repo clone' as root):
     ...
     rsync -aHAX --delete \\
       ...
       ${ADMIN_USER}@<server-ip>:/tmp/doable-stage/
     # On the server:
...
6. Cloudflare Tunnel (one tunnel per env). Skip if you passed
   DOABLE_CF_TUNNEL_UUID + DOABLE_CF_CERT_PEM_{PATH,B64} — config.yml is
   already wired:
...
7. Database migrations (after the repo is on disk):
     # Phase 9.5 has already renamed any stale 055_api_key_tool_scoping.sql
     # to 067_*. Read the DB password from the secrets file:
...
8. Start the platform (per-service systemd units, no tmux required):
     sudo systemctl start doable.target
     sudo systemctl status doable-api doable-web doable-ws

   Tail logs (replace with -f to follow):
     sudo journalctl -u doable-api -f
     sudo journalctl -u doable-web -f
     sudo journalctl -u doable-ws  -f

   Convenience CLI (if you installed the doable management tool):
     sudo doable status         # shows all 3 unit states + key health checks
     sudo doable logs api       # equivalent to journalctl -u doable-api -f
     sudo doable restart web    # systemctl restart doable-web
...
ok "setup-server-v3.sh complete."
